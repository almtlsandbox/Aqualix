...
class ImageProcessor:
    ...
    def get_water_type(self, img: np.ndarray) -> tuple:
        ...
        Utilise la même logique que l'auto-tune pour classifier l'environnement:
        - Lac/eau douce: dominance verte (G_ratio > 0.4)
        - Océan/eau profonde: faible ratio bleu (B_ratio < 0.25)  
        - Eaux tropicales: faible ratio rouge (R_ratio < 0.2)
        - Eau claire/contraste élevé: forte intensité des contours (edge_strength > 0.1)
        - Environnement standard/équilibré: cas par défaut
        
        Args:
            img: Image numpy array (BGR format)
            
        Returns:
            tuple: (type_technique, description_fr, description_en, methode_recommandee)
        """
        if img is None or img.size == 0:
